...
export default function Settings({ settings, onSettingsSaved }: SettingsProps) {
  ...
  async function save() {
    ...
    setMessage("设置已保存到 config/operator-settings.json。不会保存 secret、DATABASE_URL、cookie 或 token。");
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-md border border-line bg-white p-5">
        ...
        <p className="mt-2 text-sm text-slate-600">路径检查全部通过后，回到首页总览刷新状态。不要在设置中保存任何 secret 或数据库连接信息。</p>
      </section>
    </div>
  );
}
